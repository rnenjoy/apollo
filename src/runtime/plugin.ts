  ...
  for (const [key, clientConfig] of Object.entries(NuxtApollo.clients)) {
      ...
      if (!token.value) {
        if (clientConfig.tokenStorage === "cookie") {
          if (process.client) {
            ...
            token.value = requestCookies.cookie
              .split(";")
          ...
          process.client &&
          clientConfig.tokenStorage === "localStorage"
        ) {
    ...
    const httpLink = authLink.concat(
      createHttpLink({
        ...
        uri:
          (process.client && clientConfig.browserHttpEndpoint) ||
    ...
    if (
      !clients?.default &&
      !NuxtApollo?.clients?.default &&
      key === Object.keys(NuxtApollo.clients)[0]
    ) {
